<template>
  <div id="register">
    ...
    <div id="inputContainer" :class="{'showContainer': showContainer}">
      <h2 class="FormLabel">Registration</h2>
      ...
      <a class="linkToLogin" @click="this.$emit('show-login')"><small>Do you already have an account?</small></a>
      ...
      if(!this.registerSteps[this.position].pattern.test(this.inputValue)) {
        ...
        if(this.registerSteps[this.position]) {
          ...
          this.hideStep(() => {
            ...
            this.$emit('show-final-register', { 
              'visible': true, 
              username: this.registerSteps[0].value,
              email: this.registerSteps[1].value,
              password: this.registerSteps[2].value,
            })
          });
          setTimeout(() => {
              register.className = ''
